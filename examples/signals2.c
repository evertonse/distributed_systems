...
int message_ready = 0; // Flag to indicate if a message is ready
...
void handle_signal(int sig) {
    if (sig == SIGUSR1) {
        ...
        printf("Message: %s\n", message);
        message_ready = 0; // Reset the message flag
        // Acknowledge receipt of the message
        kill(getppid(), SIGUSR2); // Send acknowledgment to parent
    }
}
...
int main() {
    ...
    if (pid == 0) {
        ...
        while (1) {
            ...
            // Set the message flag and send signal to the child process
            message_ready = 1;
            kill(pid, SIGUSR1); // Notify child that a message is ready

            // Wait for acknowledgment

            pause(); // Wait for the acknowledgment signal
        }
