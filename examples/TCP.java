...
public class TCP {
  public static void main(String[] args) {
    ...
    if (argc < 1) {
      System.err.print(String.format("Usage: executable [server|client]\n"));
      return;
    }
    ...
    if (args[0].equals("server")) {
      ...
      TCPClient.start(args);
    } else {
      System.err.print("Invalid argument. Use 'server' or 'client'.\n");
      return;
    }
    return;
  }
...
class TCPClient {
  public static void start(String[] args) {
    try {
      ...
      byte[] buf =
          msg.getBytes(); // Obtendo a respresntação em bytes da mensagem
      ...
      System.out.print("[ Client waiting msg   ..............  ");
  ...
  private static void sendMessagesToServer(Socket sock) {
    try {
      ...
      System.out.println(
          "[ Digite suas mensagens (digite 'quit' para encerrar): ]");
      ...
      String user =
          System.getProperty("user.name") + "@" + localMachine.getHostName();
...
class TCPServer {
  public static void start(String[] args) {
    ...
    try {
      ...
      ServerSocket ss =
          new ServerSocket(3300, 5, InetAddress.getByName("127.0.0.1"));
      System.out.println("[OK] ]");

      System.out.print(
          "[ Aquardando pedidos de conexão    ..................  ");
      Socket sock =
          ss.accept(); // Operação bloqueante (aguardando pedido de conexão)
      System.out.println("[OK] ]");
      ...
      System.out.print(
          "[ Aguardando recebimento de mensagem   ..............  ");
      is.read(buf); // Operação bloqueante (aguardando chegada de dados)
      System.out.println("[OK] ]");

      String msg =
          new String(buf); // Mapeando vetor de bytes recebido para String
  ...
  private static void readMessagesFromClient2(Socket sock) {
    try {
      ...
      System.out.println(
          "[ Server: Aguardando recebimento de mensagens do cliente... ]");
      while ((msg = reader.readLine()) != null) {
        System.out.println(" Server: Mensagem recebida: " + msg);
      }
    } catch (IOException e) {
      System.out.println("Server: Erro ao ler mensagem do cliente: " +
                         e.getMessage());
    } finally {
      try {
        sock.close(); // Close the socket when done
      } catch (IOException e) {
        System.out.println(" Server: Erro ao fechar o socket: " +
                           e.getMessage());
      }
    }
  }
